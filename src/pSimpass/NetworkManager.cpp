...
NetworkManager::NetworkManager(QObject *parent) : QObject(parent), _printSentMessages(false), _printReceivedMessages(false), _printReceivedMessagesFilterEnabled(false), _printReceivedMessagesError(false)
{
    this->Init();
}

NetworkManager::NetworkManager(const QString &host, unsigned short tcpPort, unsigned short udpPort, QObject *parent) : QObject(parent), _printSentMessages(false), _printReceivedMessages(false), _printReceivedMessagesError(false)
{
    this->Init();
...
void NetworkManager::ProcessMessageTest(const QString &type, const std::vector<QString> &msg)
{
    try
    {
        ...
        if (messageName == "LATLONG")
            this->ProcessShipPosition(msg);
            ...
            this->ProcessShipHeading(msg);
        else if (messageName == "SPEED")
            this->ProcessShipSpeed(msg);
        ...
        // else if (messageName == "RRUDDER")
        //     this->ProcessOwnShipRudderAngle(msg);
...
void NetworkManager::ProcessShipHeading(const std::vector<QString> &msg)
{
    //
    // int id = msg.at( 1 ).toInt();
    // float heading = msg.at( 2 ).toFloat();

    // emit ShipHeadingChanged( id, heading );
    // //
    std::vector<QString> vaMsg = msg;
    MoosConnect::getInstancePtr()->ProcessHeading(this->ConvertStdVectorInQStringList(vaMsg));
}

void NetworkManager::ProcessOwnShipRudderAngle(const std::vector<QString> &msg)
{
    // float angle = msg.at( 1 ).toFloat();

    // emit OwnShipRudderAngleCommandChanged( angle );
    std::vector<QString> vaMsg = msg;
    MoosConnect::getInstancePtr()->ProcessRRudder(this->ConvertStdVectorInQStringList(vaMsg));
}

void NetworkManager::ProcessShipSpeed(const std::vector<QString> &msg)
{
    // int id = msg.at( 1 ).toInt();
    // float speed = msg.at( 2 ).toFloat();

    // emit ShipSpeedChanged( id, speed );
    std::vector<QString> vaMsg = msg;
    MoosConnect::getInstancePtr()->ProcessSpeed(this->ConvertStdVectorInQStringList(vaMsg));
}
...
void NetworkManager::ProcessShipPosition(const std::vector<QString> &msg)
{
    // int id = msg.at( 1 ).toInt();
    ...
    // emit ShipPositionChanged( id, latitude, longitude );
    std::vector<QString> vaMsg = msg;
    MoosConnect::getInstancePtr()->ProcessLatLong(this->ConvertStdVectorInQStringList(vaMsg));
}
